...
router.post('/', auth.verifyJWT, (req, res) => {
  ...
  User.find({ teams: team._id }, (err, user) => { // find user to see if whitelisted
    if (err) {
      handler(false, 'Failed to load user of team.', 503)(req, res);
    } else if (!user) {
      handler(false, 'Failed to find user of team.', 503)(req, res);
    } else {
      const white_listed = user.registrationWhitelist,
            registration_open = process.env.REGISTRATION === 'true';
      if (!registration_open && !white_listed) {
        handler(false, 'Cannot create team after registration.', 400)(req, res);
      } else {
          ...
          if (err) {
            ...
            async.parallel(tasks, (err, results) => {
              if (err) handler(false, 'Failed to updated existing members.', 503)(req, res);
                ...
                if (newMembers.length > 0 && !registration_open && !white_listed) {
                  handler(false, 'Cannot add students after registration.', 400)(req, res);
                } else {
                  Student.create(newMembers, (err, members) => {
                    if (err) handler(false, 'Failed to create new members.', 503)(req, res);
                    else {
                      if (goneMembers.length > 0 && !registration_open && !white_listed) {
                        handler(false, 'Cannot remove students after registration.', 400)(req, res);
                      } else {
  ...
  Team.findById(team_id, (err, team) => {
    if (err) handler(false, 'Failed to load team.', 503)(req, res);
      ...
      if (req.user.teams.indexOf(team._id.toString()) === -1) {
        ...
        // check if registration is open and whether whitelisted or not
        User.find({ teams: team._id }, (err, user) => { // find user to see if whitelisted
          if (err) {
            handler(false, 'Failed to load user of team.', 503)(req, res);
          } else if (!user) {
            ...
            const white_listed = user.registrationWhitelist,
                  registration_open = process.env.REGISTRATION === 'true';
            if (!registration_open && !white_listed) {
              handler(false, 'Cannot delete team after registration.', 400)(req, res);
            } else {
